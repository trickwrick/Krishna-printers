...
const PlateStockStatements = () => {
  ...
  const loadTransactions = async () => {
    ...
    try {
      const txRes = await fetch(`${API_BASE_URL}/api/plate-stock/transactions`);
      if (!txRes.ok) {
  ...
  const filteredTransactions = transactions.filter((item) => {
    ...
    const matchesSearch =
      (item.plateName || '').toLowerCase().includes(query) ||
    ...
    <div className="w-full px-4 mt-8 pb-12 text-gray-800 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center gap-5">
          ...
          <div>
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Total Plates Added</p>
            <h3 className="text-2xl font-black text-gray-900 tracking-tight">{totalAdded.toLocaleString()} Plates</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex items-center gap-5">
          ...
          <div>
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Total Plates Deducted</p>
            <h3 className="text-2xl font-black text-gray-900 tracking-tight">{totalDeducted.toLocaleString()} Plates</h3>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-3 mb-6">
        <div className="flex border-b border-gray-100 bg-white p-2 rounded-2xl shadow-sm gap-2 flex-1">
          {['Company plate', 'Party plate'].map((tab) => (
            <button
              key={tab}
            ...
            >
              {tab === 'Company plate' ? '🏢 Company Plate' : '🎉 Party Plate'}
            </button>
          ))}
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-50 flex flex-col sm:flex-row justify-between items-center gap-4">
          <h2 className="text-lg font-bold text-gray-900">Plate Transaction History</h2>
          <div className="relative w-full sm:w-72">
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
            <input
              type="text"
              placeholder="Search plate, party, job..."
              value={searchQuery}
          ...
          <table className="w-full text-left whitespace-nowrap">
            <thead>
              <tr className="bg-gray-50/50 text-[11px] font-black uppercase text-gray-900 tracking-[0.12em] border-b border-gray-100">
                ...
                <th className="px-6 py-4">Plate Name</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr><td colSpan="6" className="px-6 py-20 text-center text-gray-400 font-bold animate-pulse uppercase">Loading...</td></tr>
              ) : error ? (
                <tr>
                  ...
                  <tr key={item._id} className="hover:bg-gray-50/60 transition-colors">
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="font-black text-gray-900 uppercase text-sm">{item.plateName}</p>
                      </div>
                    </td>
                    <td className="px-6 py-5 text-center">
                      {item.transactionType === 'add' ? (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-700 rounded-full text-[10px] font-black uppercase ring-1 ring-emerald-100">
                          <ArrowUpCircle size={12} /> Plate Added
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-red-50 text-red-600 rounded-full text-[10px] font-black uppercase ring-1 ring-red-100">
                          <ArrowDownCircle size={12} /> Plate Deducted
                        </span>
                      )}
                    </td>
                    <td className="px-6 py-5 text-right">
                      <p className={`font-black text-sm ${item.transactionType === 'add' ? 'text-emerald-600' : 'text-red-500'}`}>
                        {item.transactionType === 'add' ? '+' : '-'}{item.quantity?.toLocaleString()} Plates
                      </p>
                    </td>
                    <td className="px-6 py-5">
                      {item.transactionType === 'deduct' && item.partyName ? (
                        ...
                        <span className="text-xs font-semibold text-gray-400 italic">
                          {item.note || 'Plate feed'}
                        </span>
                      )}
                    </td>
                    <td className="px-6 py-5 text-right">
                      <p className="font-black text-gray-900 text-sm">{item.balanceAfter?.toLocaleString()} Plates</p>
                    </td>
                  </tr>
                ))
              )}
...
export default PlateStockStatements;
